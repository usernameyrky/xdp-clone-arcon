...
xrdp_wm_ok_clicked(struct xrdp_bitmap *wnd)
{
    ...
    if (combo != 0)
    {
        ...
        if (mod_data != 0)
        {
            ...
            LOG(LOG_LEVEL_DEBUG, mod_data->values);
            LOG(LOG_LEVEL_DEBUG,wm->mm->login_values);
            LOG(LOG_LEVEL_DEBUG,mod_data->names);
            LOG(LOG_LEVEL_DEBUG, wm->mm->login_names);
        }
    }
    else
    {
...
xrdp_login_wnd_create(struct xrdp_wm *self)
{
    ...
    LOG(LOG_LEVEL_DEBUG, "Inside the login screen, Debugged By ramani");
    /* labels and edits.
